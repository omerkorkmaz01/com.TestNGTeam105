...
public class C02_SoftAssert {
    ...
    public void test01(){
        ...
        softAssert.assertEquals(actualDropdown,expectedDropdown,"eurozone secimi testi failed");
        ...
        String[] arr={"Select One", "Australia (dollar)", "Canada (dollar)",
                "Switzerland (franc)","China (yuan)",
                "Denmark (krone)","Eurozone (euro)",
                "Great Britain (pound)","Hong Kong (dollar)",
                "Japan (yen)","Mexico (peso)",
                "Norway (krone)","New Zealand (dollar)",
                "Sweden (krona)","Singapore (dollar)",
                "Thailand (baht)"};

        List<String> expectedOptionsListesi= Arrays.asList();
